...
################################################################################
def main():
  usage = 'usage: %prog [options] <fasta_file> <targets_file> <genes_bed_file>'
  ...
  parser.add_option('--prefix', dest='prefix',
      ...
      help='label of tfr files [Default: %None]')
  (options, args) = parser.parse_args()

  if len(args) != 2:
    parser.error('Must provide FASTA, target and BED files.')
  else:
    fasta_file = args[0]
    targets_file = args[1]
#    genes_bed_file = args[2]
  ...
  if not options.restart:
    ...
    #chrom_contigs = load_chromosomes(genes_bed_file)
  ...
  for fold_set in fold_labels:
    ...
    while tfr_start <= fold_set_end:
      if options.prefix is not None:
        tfr_stem = '%s/%s-%s%d' % (tfr_dir, fold_set, options.prefix, tfr_i)
      else:
      ...
      if options.restart and os.path.isfile(tfr_file):
        ...
        cmd = 'basenji_write_genes.py'
        cmd += ' -s %d' % tfr_start
        cmd += ' -e %d' % tfr_end
        cmd += ' --umap_clip %f' % options.umap_clip
        cmd += ' -x %d' % options.crop_bp
        cmd += ' --len %d' % (options.seq_length//options.pool_width)
...
################################################################################
def load_chromosomes(genes_bed_file):
  """ Load genes segments from either a bed file or
          \t seperated table. """
  chrom_segments = {}
  for line in open(genes_bed_file):
    a = line.split()
    chrom_segments[a[0]] = [(int(a[1]), int(a[2]))]
